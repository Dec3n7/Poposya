...
class Settings(BaseSettings):
    ...
    # АБСОЛЮТНЫЙ потолок жизни веб-сессии от входа (часы): даже при активности
    # сессия живёт не дольше. Скользящее продление (ниже) потолок не двигает.
    web_session_ttl_hours: int = 24
    # Скользящий тайм-аут бездействия (минуты, 0 = выключено). При каждом запросе
    # к панели кука перевыпускается с окном idle; простой дольше него истекает на
    # СЕРВЕРЕ (exp в JWT), а не только гасит вкладку в JS — то есть даже
    # украденный токен мёртв после idle бездействия. Заодно ограничивает окно
    # устаревания вшитых в токен прав Discord до этого значения (не до 24 ч).
    web_idle_ttl_minutes: int = 15
    ...
    # v2: закрывает легаси-токены без маски прав (permissions=None), которые
    # раньше трактовались как «всё можно» — после бампа такие токены не пройдут
    # decode, пользователи перелогинятся и получат строгую маску прав.
    web_session_version: int = 2
    # Максимальный возраст вшитого в токен СНИМКА прав Discord (минуты, 0 =
    # выключено). Права берутся из OAuth один раз при входе и живут весь TTL
    # сессии; idle-таймаут выше ограничивает их устаревание только для
    # НЕактивной сессии. Этот потолок ограничивает и активную: если снимок прав
    # старше него, привилегированные ДЕЙСТВИЯ (бан/кик/тайм-аут/роли/настройки)
    # требуют перелогина (чтения не трогаются). Так разжалованный в Discord
    # админ теряет доступ к действиям в панели за минуты, а не за сутки. Личность
    # (кто ты) и операторские права (web_operator_ids) от этого не зависят.
    web_perm_ttl_minutes: int = 0
    ...
    # --- монетизация / тарифы ---
    # тариф для серверов БЕЗ явной подписки (выданной оператором). "free" =
    # enforcement ВКЛючён: серверы без подписки получают Free — их лимиты
    # клампятся, а Premium-модули гейтятся (require_tier/tier_allows в когах).
    # Выданные вручную в панели подписки работают до своего срока.
    ...
    # warden_api_token — read-уровень (GET /status). warden_control_token —
    # управление (POST /pause, /resume). Пусто = разделения нет, управление
    # идёт по read-токену (совместимость с одним общим секретом). Значения
    # должны совпадать с WARDEN_API_TOKEN / WARDEN_CONTROL_TOKEN у сторожа.
    warden_api_url: str = ""
    warden_api_token: str = ""
    warden_control_token: str = ""

    @property
    def warden_control_token_effective(self) -> str:
        """Токен для управляющих вызовов сторожа: отдельный control-токен, если
        задан, иначе read-токен (обратная совместимость с одним секретом)."""
        return self.warden_control_token or self.warden_api_token
