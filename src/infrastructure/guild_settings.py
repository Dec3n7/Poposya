...
_LABELS: dict[str, tuple[str, str]] = {
    ...
    "warn_expire_days": ("Затухание варнов (0 = не затухают)", "дн"),
    "warn_escalation": ("Эскалация: рецидив ужесточает наказание", ""),
    "warn_ban_minutes": ("Tempban рецидивисту (3-е+ достижение порога)", "мин"),
    "moderation_dm_notice": ("Слать наказанному ЛС с причиной", ""),
    "spam_limit": ("Сообщений за окно = спам", ""),
    "spam_window": ("Окно антиспама", "сек"),
    "spam_mute_minutes": ("Мут за спам", "мин"),
    "spam_mention_limit": ("Масс-упоминаний в сообщении → мут (0 = выкл)", ""),
    "spam_block_invites": ("Удалять чужие инвайты от не-модеров", ""),
    # отношения и роли
    ...
    "main_channel_id": ("Главный канал (0 = авто: system-канал)", ""),
    "welcome_channel_id": ("Канал приветствий/прощаний (0 = авто)", ""),
    "album_channel_id": ("Канал «Альбом» (0 = выкл)", ""),
    ...
    # апелляции
    "appeals_channel": ("Канал апелляций (0=выкл)", ""),
    # находки
    ...
    # кросс-серверные баны
    "banwatch_enabled": ("Кросс-серверные баны (весь модуль)", ""),
    "digest_enabled": ("Еженедельный дайджест (весь модуль)", ""),
    "appeals_enabled": ("Апелляции наказаний (весь модуль)", ""),
    "banwatch_threshold": ("Порог «отмеченного» (забанен на N серверах)", ""),
    ...
    "achievements_enabled": ("Достижения (весь модуль)", ""),
}
...
FEATURE_MODULES: tuple[ModuleSpec, ...] = (
    ...
    ModuleSpec(
        key="banwatch",
        label="Кросс-серверные баны",
        master="banwatch_enabled",
        subs=(),
        description=(
            "Показывает модератору, что участник забанен на других серверах бота "
            "(с причинами) — команда /checkuser и раздел во вкладке «Модерация» "
            "панели. Наружу в Discord ничего не публикуется, видно только админам. "
            "Тумблер гасит показ на этом сервере; сбор банов идёт по всем серверам."
        ),
    ),
    ModuleSpec(
        key="moderation",
    ...
    ModuleSpec(
        key="achievements",
        label="Достижения",
        master="achievements_enabled",
        subs=(),
        description=(
            "Достижения участников (отношения, находки, музыка, войс): при открытии "
            "бот объявляет ачивку карточкой в основном канале, команда /achievements "
            "показывает витрину. Premium-модуль. Выключение гасит и уведомления, и "
            "витрину на этом сервере."
        ),
    ),
    ModuleSpec(
        key="digest",
        label="Еженедельный дайджест",
        master="digest_enabled",
        subs=(),
        description=(
            "Раз в неделю (воскресенье вечером) бот постит в заданный канал итоги "
            "недели голосом персоны: активность, войс, новенькие, дни рождения, "
            "киноклуб. Канал — настройка «Канал недельного дайджеста»; при сбое ИИ "
            "публикуется шаблонный текст без ИИ."
        ),
    ),
    ModuleSpec(
        key="appeals",
        label="Апелляции наказаний",
        master="appeals_enabled",
        subs=(),
        description=(
            "Наказанному (бан/темпбан/мут) в ЛС приходит кнопка «Обжаловать»; "
            "апелляция уходит в очередь модерации (канал апелляций + вкладка в "
            "панели), где модератор принимает — наказание снимается — или "
            "отклоняет. Канал — настройка «Канал апелляций»."
        ),
    ),
)
