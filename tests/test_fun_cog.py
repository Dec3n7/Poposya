...
def make_rank(**over):
    base = dict(points=300, level=3, role_index=1, is_exclusive=False, frozen=False,
                next_threshold=450, user_notes="", survey=SurveyData(),
                birthday_day=None, birthday_month=None, deep_dialogs=0,
                last_dialog_at=None)
    base.update(over)
...
def make_cog(relationship=None, activity=None, settings=None, **kw):
    bot = MagicMock()
    return FunCog(bot, activity or make_activity(), relationship or make_relationship(),
                  None, settings or make_settings(), **kw)
...
async def test_dice_duel_has_winner(monkeypatch):
    ...
    monkeypatch.setattr("src.infrastructure.discord.cogs.fun.random.randint",
...
async def test_profile_builds_embed():
    ...
    relationship.get_rank.execute.return_value = make_rank(
        level=5, points=800, role_index=4, deep_dialogs=6,
        survey=SurveyData(interests="Игры", season="лето", completed=True),
        birthday_day=2, birthday_month=6, is_exclusive=False)
