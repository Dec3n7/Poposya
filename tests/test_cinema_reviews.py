...
def make_entry(**over):
    base = dict(
        guild_id=10, title="Фильм", added_by=1, added_at=NOW, status="rating", rating_message_id=500
    )
    base.update(over)
...
async def _seed_rating_entry(uow_factory, rating_message_id=500, status="rating"):
    async with uow_factory() as uow:
        entry = await uow.movies.add(make_entry(rating_message_id=rating_message_id, status=status))
        await uow.commit()
        return entry
