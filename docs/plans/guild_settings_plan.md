...
Статус: ✅ **РЕАЛИЗОВАНО** (план от 2026-07-11). Пер-серверные настройки живут в
`src/application/guild_config`, управляются командой `/config` и вкладками
«Настройки»/«Модули» веб-панели; модули включаются/выключаются на каждом сервере.
Файл оставлен как исторический план этапа.

> **Решение:** [ADR-008](../adr/ADR-008-per-guild-settings-provider.md).
