...
**Тарифицируем в музыке ТОЛЬКО не-YouTube (законно продавать)** — прямым тарифным
чеком в `MusicCog` (лайки per-user и караоке-live не ложатся в per-guild-схему
клампа, поэтому не через `TIERABLE`/`MODULE_MIN_TIER`, а `_is_free(gid)`):
- **Лайки** — личная коллекция, не через YouTube: free ≤20 (`_FREE_LIKED_MAX`),
  Premium/Pro — полный `music_liked_max_per_user` (300). Гейт по тарифу гильдии,
  где нажали ❤️; при упирании — нудж на `/premium`.
- **Караоке-live** (`/lyrics live`) — текст с lrclib, не YouTube: Premium.
  Статичный `/lyrics` остаётся free.

**Сознательно НЕ трогаем (это ЮТУБ — продавать нельзя / против логики):**
- **Плеер, очередь, `/seek`, радио, плейлисты** — играют YouTube-треки; музыка
  как таковая = бесплатный бонус (§B идей), не headline-paywall. Оставлены free.
- **Активный лимит `/remind`** — такой настройки нет; «мёртвый» ключ ради
  тарификации не заводим (при необходимости — по рецепту §3.1).
