...
> ⚠️ **Актуальная пошаговая процедура — в [../docs/DEPLOY.md](../docs/DEPLOY.md)
> (шаг 7).** Этот файл — фоновая справка по вариантам TLS. Учтите: nginx уже
> переведён на rootless и слушает `8443 ssl` (не `:80`), а сертификаты читает из
> `/etc/nginx/certs/{fullchain,privkey}.pem` — для прода достаточно подменить эти
> два файла на Let's Encrypt (блок 443 раскомментировать НЕ нужно, он уже живой).

Раздел ниже описывает более раннюю схему (`:80`/`8081`) и оставлен для контекста.
